...
function getTable(config) {
	return new Promise((resolve, reject) => {
		...
		request(options, (error, response, body) => {
			if (error || (response && response.statusCode !== 200)) return reject(error || new Error(`Request failed: ${response && response.statusCode}`));
			...
			function factory(temp) {
				return new Promise((resolveFactory, rejectFactory) => {
					request(utils.image_options(temp[property[0]]), (error, response, body) => {
						if (error || (response && response.statusCode !== 200)) {
							rejectFactory(error || new Error(`Image request failed: ${response && response.statusCode}`));
							return;
						}
			...
			Promise.allSettled(queue.map(temp => factory(temp))).then(results => {
				...
				if (counter++ < config.pages) {
					// chain the recursive call so the returned promise resolves when the entire paging is done
					getTable({
